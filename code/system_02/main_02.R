...
user_ids <- as.factor(paste0('u', ratings$UserID))
movie_ids <- as.factor(paste0('m', ratings$MovieID))
rating_vals <- ratings$Rating

r_sparse <- sparseMatrix(
  i = as.integer(user_ids),
  j = as.integer(movie_ids),
  x = rating_vals)

rownames(r_sparse) = levels(user_ids)
colnames(r_sparse) = levels(movie_ids)
